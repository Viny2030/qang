...
\date{\small Draft of 23 September 2026}
...
removes the ambiguity in 23 of 24 circuits tested. Every number is reproduced by a script and
...
including a device-calibrated validation. Section~\ref{sec:limits} lists the limitations
together.
...
(viii) The device result is a calibration-based simulation. A run on real hardware
(\texttt{--mode ibm}) is prepared but has not yet been performed.

\section{Code availability}\label{sec:code}

The library, every example, and the regression tests pinning every number above are available at
